...
def alg(sc, data_set_rdd, threshold, randomized=True):
    import pdb
    pdb.set_trace()
    data_set_size = data_set_rdd.count()
    partitions_num = data_set_rdd.getNumPartitions()
    sample_size = _calculate_sample_size(threshold, data_set_size) if randomized else data_set_size
    sample = data_set_rdd.takeSample(False, sample_size)
    sample = sc.parallelize(sample, partitions_num)
    sample.cache()
    ...
    while candidates:
        ...
        start = time.time()
        candidates = filter(lambda candidate: candidates_sizes[json.dumps(list(candidate))] >= scaled_threshold, candidates)
        end = time.time()
...
DELTA = 0.1
ALPHA = 0.1
def _calculate_sample_size(threshold, data_set_size):
    return math.ceil((math.ln(1 / EPSILON) * 2 * data_set_size) / ((1 - ALPHA) * DELTA ** 2 * threshold))
